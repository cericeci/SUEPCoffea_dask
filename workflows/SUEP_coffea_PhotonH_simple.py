...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def selectByTrigger(self, events, extraColls = []):
        ...
        if self.era == 2018:
           cutAnyHLT = events.HLT.Photon200 | events.HLT.Photon110EB_TightID_TightIso |  events.HLT.Photon100EB_TightID_TightIso |  events.HLT.Photon120_R9Id90_HE10_IsoM | events.HLT.Photon75_R9Id90_HE10_IsoM | events.HLT.Photon50_R9Id90_HE10_IsoM | events.HLT.Photon30_HoverELoose | events.HLT.Photon20_HoverELoose
           return events[cutAnyHLT], [coll[cutAnyHLT] for coll in extraColls]
        if self.era == 2017:
           cutAnyHLT = events.HLT.Photon200 |  events.HLT.Photon165_R9Id90_HE10_IsoM |  events.HLT.Photon120_R9Id90_HE10_IsoM | events.HLT.Photon75_R9Id90_HE10_IsoM | events.HLT.Photon50_R9Id90_HE10_IsoM | events.HLT.Photon30_HoverELoose | events.HLT.Photon20_HoverELoose
           return events[cutAnyHLT], [coll[cutAnyHLT] for coll in extraColls]
        if self.era == 2016 or self.era == 2015: # 2015==2016APV
           cutAnyHLT = events.HLT.Photon175 | events.HLT.Photon165_HE10 | events.HLT.Photon165_R9Id90_HE10_IsoM | events.HLT.Photon90_R9Id90_HE10_IsoM | events.HLT.Photon75_R9Id90_HE10_IsoM | events.HLT.Photon50_R9Id90_HE10_IsoM |  events.HLT.Photon36_R9Id90_HE10_IsoM |  events.HLT.Photon30_R9Id90_HE10_IsoM | events.HLT.Photon22_R9Id90_HE10_IsoM
           return events[cutAnyHLT], [coll[cutAnyHLT] for coll in extraColls]
        return events, [coll[cutAnyHLT] for coll in extraColls]

    def getTriggerBits(self, events):
        if self.era == 2018:
            out = (events.HLT.Photon200 ) + ( events.HLT.Photon110EB_TightID_TightIso )*2 + (  events.HLT.Photon100EB_TightID_TightIso )*4 + (  events.HLT.Photon120_R9Id90_HE10_IsoM )*8 + ( events.HLT.Photon75_R9Id90_HE10_IsoM )*16 + ( events.HLT.Photon50_R9Id90_HE10_IsoM )*32 + ( events.HLT.Photon30_HoverELoose )*64 + ( events.HLT.Photon20_HoverELoose )*128
        if self.era == 2017:
            out = (events.HLT.Photon200 ) + (  events.HLT.Photon165_R9Id90_HE10_IsoM )*2 + (  events.HLT.Photon120_R9Id90_HE10_IsoM )*4 + ( events.HLT.Photon75_R9Id90_HE10_IsoM )*8 + ( events.HLT.Photon50_R9Id90_HE10_IsoM )*16 + ( events.HLT.Photon30_HoverELoose )*32 + ( events.HLT.Photon20_HoverELoose)*64  
        if self.era == 2016 or self.era == 2015: # 2015==2016APV
            out = (events.HLT.Photon175 ) + ( events.HLT.Photon165_HE10 )*2 + ( events.HLT.Photon165_R9Id90_HE10_IsoM )*4 + ( events.HLT.Photon90_R9Id90_HE10_IsoM )*8 + ( events.HLT.Photon75_R9Id90_HE10_IsoM )*16 + ( events.HLT.Photon50_R9Id90_HE10_IsoM )*32 + (  events.HLT.Photon36_R9Id90_HE10_IsoM )*64 + (  events.HLT.Photon30_R9Id90_HE10_IsoM )*128 + ( events.HLT.Photon22_R9Id90_HE10_IsoM)*256
        return out
    ...
    ###lepton selection criteria--4momenta collection for plotting
        ...
        cutNoLepton = ((ak.num(selMuons, axis=1) + ak.num(selElectrons, axis=1)) < 1)
        return events[cutNoLepton]

    def selectByPhotons(self, events, extraColls = []):
        photons = ak.zip({
            "pt": events.Photon.pt,
            "eta": events.Photon.eta,
            "phi": events.Photon.phi,
            "mass": events.Photon.mass,
            "pdgId": events.Photon.pdgId,
          }, with_name="Momentum4D")
        ...
        ###  Muons: loose ID + dxy dz cuts mimicking the medium prompt ID https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2
        ###  Electrons: loose ID + dxy dz cuts for promptness https://twiki.cern.ch/twiki/bin/view/CMS/EgammaCutBasedIdentification
        cutPhotons     = (events.Photon.mvaID_WP90) & (events.Photon.pt >= 30) & (abs(events.Photon.eta) <= 2.5) & (events.Photon.electronVeto)
        selPhotons = photons[cutPhotons]
        cutOnePhoton = (ak.num(selPhotons, axis=1) == 1)
        return events[cutOnePhoton], selPhotons[cutOnePhoton]
    ...
    def selectByJets(self, events, leptons = [],  altJets = [], extraColls = []):
        ...
        # Minimimum pT, eta requirements + jet-lepton (photon) recleaning. We set minimum pT at 20 here to allow for the syst variations later
        jetCut = (Jets.pt > 20) & (abs(Jets.eta)<2.5) & (Jets.deltaR(leptons[:,0])>= 0.4)  & (Jets.jetId >= 6)
        jets = Jets[jetCut]
	...
	### LOST TRACKS ###
        ...
        # Sorting out the tracks that overlap with leptons (photons)
        totalTracks = totalTracks[(totalTracks.deltaR(leptons[:,0])>= 0.4)]
    ...
    def process(self, events):
        ...
        # Lepton selection
        self.events = self.selectByLeptons(self.events)
        if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator # If we have no events, we simply stop

        # Trigger selection
        if debug: print("%i events pass lepton cuts. Applying trigger requirements...."%len(self.events))
        self.events = self.selectByTrigger(self.events)[0]


        if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
        if debug: print("%i events pass trigger cuts. Selecting photons..."%len(self.events))

        # Photon selection
        self.events, self.photons = self.selectByPhotons(self.events)
        if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator # If we have no events, we simply stop
        if debug: print("%i events pass photon cuts. Selecting jets..."%len(self.events))


        # Jet selection
        self.events, self.jets = self.selectByJets(self.events, self.photons)[:2] # Photons are needed to do jet-photon cleaning
	# Sorting jets by pt.
        ...
        if self.doTracks:
            ...
            self.events, self.tracks = self.selectByTracks(self.events, self.photons)[:2] # Again, we need photons to clean the tracks
            if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
        ...
        if self.isMC:
          ...
          #self.triggerSFs  = self.doTriggerSFs(self.photons)
          self.photonSFs   = self.doPhotonSFs(self.photons)
        # ------------------------------------------------------------------------------
        # ------------------------------- UNCERTAINTIES --------------------------------
        # ------------------------------------------------------------------------------
        ...
        for var in self.varsToDo:
            ...
            if self.doTracks:
                ...
                if self.doClusters:
                    ...
                    #cutPhopt = (ak.max(self.photons.pt, axis=1) > 200)
                    #self.applyCutToAllCollections(cutPhopt)
                    #if not(self.shouldContinueAfterCut(self.events, outputs)): continue
                    #if debug: print("%i events pass Photonpt cuts. Doing more stuff..."%len(self.events))
    ...
    def applyCutToAllCollections(self, cut): # Cut has to by a selection applicable across all collections, i.e. something defined per event
        ...
        if self.isMC:
            ...
            for var in self.photonSFs:
                self.photonSFs[var]   = self.photonSFs[var][cut]
        if self.doTracks:
    ...
    def saveAllCollections(self): # Save collections before cutting, to reset when dealing with systematics
        ...
        if self.isMC:
            ...
            self.safephotonSFs = {}
            for var in self.safephotonSFs:
                self.safephotonSFs[var]   = self.photonSFs[var]
    ...
    def resetAllCollections(self): # Reset collections to before cutting, useful when dealing with systematics
        ...
        if self.isMC:
            ...
            for var in self.photonSFs:
                self.photonSFs[var] = self.safephotonSFs[var]
    ...
    def doTriggerSFs(self, photons):
        return ak.ones_like(photons[:,0])

    def doPhotonSFs(sphof, photons):
        if sphof.era == 2015:
           tag = "16APV"
           etag = "2016preVFP"
        elif sphof.era == 2016:
        ...
        phos, nphos = ak.flatten(photons), np.array(ak.num(photons))


        phoall  = correctionlib.CorrectionSet.from_file("data/EGammaUL%s/photon.json"%tag)["UL-Photon-ID-SF"]

        phoSF     = phoall.evaluate(etag,"sf","wp90", phos.eta, phos.pt)
        phoSFUp   = phoall.evaluate(etag,"sfup","wp90", phos.eta, phos.pt)
        phoSFDown = phoall.evaluate(etag,"sfdown","wp90", phos.eta, phos.pt)

        phoSF     = ak.unflatten(phoSF, nphos)
        phoSFUp   = ak.unflatten(phoSFUp, nphos)
        phoSFDown = ak.unflatten(phoSFDown, nphos)

        PhoSF = {}
        PhoSF["PhotonSF"]     = phoSF
        PhoSF["PhotonSFUp"]   = phoSFUp
        PhoSF["PhotonSFDown"] = phoSFDown
        return PhoSF
    ...
    def doAllPlots(self, channel, debug=True):
        ...
        out["Photon_pt"]  = self.photons.pt[:,0]
        out["Photon_eta"] = self.photons.eta[:,0]
        out["Photon_phi"] = self.photons.phi[:,0]
        out["Photon_m"]   = self.photons.mass[:,0]
        # Trigger paths
        out["HLT_path"]   = self.getTriggerBits(self.events)
        ...
        if self.isMC:
           ...
           out["PhotonSF"]  = self.photonSFs["PhotonSF"][:]
        ...
        if self.var == "" and self.do_syst:
           ...
           out["PhotonSF_Up"]  = self.photonSFs["PhotonSFUp"][:]
           out["PhotonSF_Dn"]  = self.photonSFs["PhotonSFDown"][:]
    ...
    def getAllMassPairsMode(self, tracks, modeBins=[], minn=0, maxx=20):
        ...
        for iEv in range(ak.num(tracks, axis=0)):
          ...
          for v in modeBins:
            ...
            if first:
                print(len(masses[iEv,:]), len(FourMass[iEv,:]))
                print(len(masses[iEv,:]), masses[iEv,:])
                print(len(indexes[iEv,:]), indexes[iEv,:])
                print(len(matchMassCut[iEv,:]), matchMassCut[iEv,:])
                print(len(fInd[iEv,:]), fInd[iEv,:])
                print(len(FourMass[iEv,:]), FourMass[iEv,:])
                print(vals[v], valsf[v])
                print(hb, hff)
                first = False
